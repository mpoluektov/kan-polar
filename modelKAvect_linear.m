function [ yhat_all, LgradB_all, LgradT_all, t_min, t_max ] = modelKAvect_linear( x, xmin, xmax, ymin, ymax, fnB, fnT )
...
Cprnm = kron(eye(p*r),ones(1,n*m));

fnB_r = reshape(fnB,n*m,p);
fnT_r = reshape(fnT,q*p,r);
...
%. deriv.
top = Ud * ( kron(fnT_r,ones(1,p)) .* repmat(Cpq,1,r) ) * (q-1) / ( tmax - tmin );
V_ex = repmat(V,1,p*r);
LgradB_all = ( top * Cprnm ) .* V_ex;
LgradT_all = repmat(U,1,r);
